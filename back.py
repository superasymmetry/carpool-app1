...
def store_login_data(uname, psword, fname, lname, eml, res, tel=None):
    ...
    c.execute("INSERT INTO carpool1 (username, psword, fname, lname, telephone, email, residence) VALUES (?, ?, ?, ?, ?, ?, ?)", (uname, psword, fname, lname, tel, eml, res))
    conn.commit()
    conn.close()
...
def validate_credentials(uname, psword):
    ...
    c = conn.cursor()
    try:
        c.execute("SELECT * FROM carpool1 WHERE username= ANY(%s)", [[uname]])
        usr = c.fetchone()
        print(usr)
        if usr[1]==psword:
            conn.close()
...
def store_date(date, user):
    ...
    conn = psycopg2.connect(database="postgres",user="postgres",password="Ab200708",host="localhost",port=5432,)
    c = conn.cursor()
    c.execute("SELECT * FROM carpool1 WHERE username=ANY(%s);", [[user]])
    row = c.fetchone()
    a=row[0]
    print(a)
    # try:
    ...
    #     return False
    update_query = "UPDATE carpool1 SET dates=%s WHERE username=%s"
